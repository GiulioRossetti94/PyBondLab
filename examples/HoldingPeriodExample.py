...
# It uses the staggered rebalancing procedure from Novy-Marx and Velikov RFS paper
# Uses the OSBAP data
...
# We will use the MMN-adjusted CS, credit spread factor as an example #
sort_var1 = 'CS'

holding = [1,2,3,4,5,6]

for h in holding:
    print('Sorting on CS with staggered holding-period set to ' + str(h))
    
    # initialize the strategy
    single_sort = pbl.SingleSort(h, 
                                 sort_var1, 
                                 n_portf,
                                 skip)
    ...
    _out.columns = ['EW_'+ str(sort_var1) +'h'+str(h) , 'VW_'+ str(sort_var1)+'h'+str(h)]  
    ...
    _out.columns = ['EW_'+ str(sort_var1) + str('_Long')+'h'+str(h) , 'VW_'+ str(sort_var1) + str('_Long')+'h'+str(h)]
    ...
    _out.columns = ['EW_'+ str(sort_var1) + str('_Short')+'h'+str(h) , 'VW_'+ str(sort_var1) + str('_Short')+'h'+str(h)]
    ...
    _out_ew.columns = ['EW_'+ str(sort_var1)+'_TO'+'h'+str(h)]
    
    _out_vw = ((vw_turn.iloc[:,0]+vw_turn.iloc[:,(n_portf-1)])/2).to_frame()
    _out_vw.columns = ['VW_'+ str(sort_var1)+'_TO'+'h'+str(h)]
