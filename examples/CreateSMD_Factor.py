# This script constructs the SMD factor from the paper:
# "Pricing Corporate Bonds with Credit Risk Primitives", by Dickerson & Nozawa
# Available here: https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4822379
...
# Create negative of modified duration #
tbl1['DUR'] = tbl1['duration']*-1
...
for h in smd_vars:
    print('Sorting on ' +str(h))
    
    # initialize the strategy
    single_sort = pbl.SingleSort(holding, 
                                 h, 
                                 n_portf,
                                 skip)
    ...
    _out.columns = ['EW_'+ str(h) , 'VW_'+ str(h)]  
    ...
    _out_ew.columns = ['EW_'+ str(h)+'_TO']
    
    _out_vw = ((vw_turn.iloc[:,0]+vw_turn.iloc[:,(n_portf-1)])/2).to_frame()
    _out_vw.columns = ['VW_'+ str(h)+'_TO']
    Strategies_LS_TO = pd.concat([Strategies_LS_TO,  _out_ew, _out_vw], axis = 1)

# Create SMD #
Strategies_LS['EW_SMD']    = (Strategies_LS['EW_CS']   +Strategies_LS['EW_DUR'])/2
Strategies_LS_TO['EW_SMD'] = (Strategies_LS_TO['EW_CS_TO']   +Strategies_LS_TO['EW_DUR_TO'])/2

Strategies_LS['VW_SMD']    = (Strategies_LS['VW_CS']   +Strategies_LS['VW_DUR'])/2
Strategies_LS_TO['VW_SMD'] = (Strategies_LS_TO['VW_CS_TO']   +Strategies_LS_TO['VW_DUR_TO'])/2
