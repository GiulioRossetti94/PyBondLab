...
class StrategyFormation:
    ...
    def _can_use_withinfirm_fast_path(self) -> bool:
        ...
        if not is_within_firm:
            return False
        # Fast path requires: no turnover, no chars
        if self.turnover:
            return False
        if self.chars:
            return False
    ...
    def _fit_withinfirm_fast(self):
        """
        Ultra-fast portfolio formation for WithinFirmSort strategy.
        ...
        2. All (date × rating × firm) groups are processed in parallel using prange
        3. Uses fast numba aggregation kernel
        4. No turnover/scaled weights computation
        5. No characteristics computation
        ...
        Note: Ranks are computed at formation date (t), returns collected at return date (t+h).
        For HP=1, formation date = return date - 1.
        For HP>1, staggered rebalancing averages across cohorts.
        """
        from .numba_core import (
            compute_within_firm_assignments_all_dates,
            compute_within_firm_aggregation_fast
        )

        path_type = "EP" if self._computing_ep else "EA"
        if self.verbose:
            print(f"Using ULTRA-FAST WithinFirmSort path ({path_type})...")

        # Get strategy parameters
        firm_id_col = getattr(self.strategy, 'firm_id_col', 'PERMNO')
        rating_bins = getattr(self.strategy, 'rating_bins', [-np.inf, 7, 10, np.inf])
        min_bonds_per_firm = getattr(self.strategy, 'min_bonds_per_firm', 2)
        sort_var_main, _ = self._get_sort_vars()

        TM = len(self.datelist)
        hp = self.hor  # Holding period
        ...
        # Add rating tercile to data
        data['rating_terc'] = rating_terc.values

        # Create bond ID mapping
        unique_bonds = data[ColumnNames.ID].unique()
        bond_to_idx = {b: i for i, b in enumerate(unique_bonds)}
        idx_to_bond = {i: b for b, i in bond_to_idx.items()}
        data['bond_idx'] = data[ColumnNames.ID].map(bond_to_idx)
        n_bonds = len(unique_bonds)

        # Create firm mapping
        unique_firms = data[firm_id_col].dropna().unique()
        firm_to_idx = {f: i for i, f in enumerate(unique_firms)}
        data['firm_idx'] = data[firm_id_col].map(firm_to_idx).fillna(-1).astype(int)
        data['date_idx'] = data[ColumnNames.DATE].map(date_to_idx)
        n_firms = len(unique_firms)

        # Step 2: Compute ranks at each FORMATION date
        # For formation dates, we need valid signal, weight, rating, firm
        formation_valid_mask = (
            data['rating_terc'].notna() &
            data[sort_var_main].notna() &
            data[ColumnNames.VALUE_WEIGHT].notna() &
            data[firm_id_col].notna()
        )
        formation_data = data.loc[formation_valid_mask].copy()

        if formation_data.empty:
            return self._create_empty_results()

        # Sort by (date, rating_terc, firm) and find group boundaries
        formation_data = formation_data.sort_values(
            [ColumnNames.DATE, 'rating_terc', firm_id_col]
        ).reset_index(drop=True)

        # Create group IDs
        formation_data['_grp'] = formation_data.groupby(
            [ColumnNames.DATE, 'rating_terc', firm_id_col], sort=False
        ).ngroup()

        # Find group boundaries
        grp_arr = formation_data['_grp'].values
        group_changes = np.concatenate([
            [0],
            np.where(np.diff(grp_arr) != 0)[0] + 1,
            [len(formation_data)]
        ])
        group_starts = group_changes[:-1].astype(np.int64)
        group_ends = group_changes[1:].astype(np.int64)

        # Convert to numpy arrays for formation data
        form_date_idx = formation_data[ColumnNames.DATE].map(date_to_idx).values.astype(np.int64)
        form_rating_terc = formation_data['rating_terc'].values.astype(np.float64)
        form_firm_idx = formation_data['firm_idx'].values.astype(np.int64)
        form_bond_idx = formation_data['bond_idx'].values.astype(np.int64)
        form_signal = formation_data[sort_var_main].values.astype(np.float64)
        form_vw = formation_data[ColumnNames.VALUE_WEIGHT].values.astype(np.float64).clip(min=1e-10)

        # Compute portfolio assignments
        portfolio_ranks = compute_within_firm_assignments_all_dates(
            date_idx=form_date_idx,
            rating_terc=form_rating_terc,
            firm_idx=form_firm_idx,
            signal=form_signal,
            vw=form_vw,
        ...
        # Store ranks in formation_data for lookup
        formation_data['ptf_rank'] = portfolio_ranks

        # Step 3: Build lookup table: (date_idx, bond_idx) -> (rank, rating_terc, firm_idx, vw)
        # This allows us to look up ranks from formation date when collecting returns
        rank_lookup = {}
        for i in range(len(formation_data)):
            d_idx = form_date_idx[i]
            b_idx = form_bond_idx[i]
            rank = portfolio_ranks[i]
            if rank > 0:
                rank_lookup[(d_idx, b_idx)] = (
                    rank,
                    form_rating_terc[i],
                    form_firm_idx[i],
                    form_vw[i]
        ...
        # Step 4: Collect returns using standard timing (formation at t, return at t+h+1)
        # For HP=1: h=0, so return at t+1
        # For HP>1: staggered rebalancing - cohort c forms at t, returns at t+h+1 for h=0...hp-1
        #
        # IMPORTANT: Results are indexed by FORMATION date (not return date)
        # This matches the slow path behavior where port_idx is keyed by formation date
        all_records = []

        # Get return data - need valid returns
        return_data = data[data[ret_col].notna()].copy()

        # NOTE: For WithinFirmSort with HP>1, the slow path only stores the LAST
        # horizon's data in port_idx (because it's overwritten in the loop).
        # To match, we only use h = hp - 1 (the last horizon).
        #
        # For HP=1: h=0, return at form_t_idx + 1
        # For HP>1: h=hp-1, return at form_t_idx + hp

        # For each formation date, get returns from return date (form + hp)
        for form_t_idx in range(TM - 1):  # Formation dates (all except last)
            t1_idx = form_t_idx + hp  # Return date (matches slow path: last h)

            if t1_idx >= TM:
                continue

            # Get bonds at return date
            t1_date = idx_to_date[t1_idx]
            ret_at_t1 = return_data[return_data[ColumnNames.DATE] == t1_date]

            for _, row in ret_at_t1.iterrows():
                b_idx = row['bond_idx']
                if np.isnan(b_idx):
                    continue
                b_idx = int(b_idx)
                ...
                key = (form_t_idx, b_idx)
                if key not in rank_lookup:
                    continue

                rank, rt, f_idx, vw_val = rank_lookup[key]

                ret_val = row[ret_col]
                if np.isnan(ret_val) or f_idx < 0 or np.isnan(rt):
                    continue

                all_records.append({
                    'date_idx': form_t_idx,  # Results indexed by FORMATION date
                    'firm_idx': f_idx,
                    'rating_terc': rt,
                    'ptf_rank': float(rank),
                    'ret': ret_val,
        ...
        # Step 5: Run fast aggregation kernel
        # NOTE: For WithinFirmSort, the slow path doesn't do cohort averaging even
        # for HP>1. It only stores the last horizon's data in port_idx, so we match
        # that behavior by collecting only h=hp-1 returns (done above).
        (ew_long_short, vw_long_short,
         ew_high_ret, ew_low_ret,
         vw_high_ret, vw_low_ret) = compute_within_firm_aggregation_fast(
            date_idx=combined_df['date_idx'].values.astype(np.int64),
            firm_idx=combined_df['firm_idx'].values.astype(np.int64),
        ...
        # Create portfolio returns DataFrames
        ew_ret = np.column_stack([ew_low_ret, ew_high_ret])
        vw_ret = np.column_stack([vw_low_ret, vw_high_ret])
        ...
        ewls_df = pd.DataFrame(ew_long_short, index=self.datelist, columns=[f'{prefix}_{self.name}'])
        vwls_df = pd.DataFrame(vw_long_short, index=self.datelist, columns=[f'{vw_prefix}_{self.name}'])
        ew_long_df = pd.DataFrame(ew_high_ret, index=self.datelist, columns=[f'LONG_{prefix}_{self.name}'])
        vw_long_df = pd.DataFrame(vw_high_ret, index=self.datelist, columns=[f'LONG_{vw_prefix}_{self.name}'])
        ew_short_df = pd.DataFrame(ew_low_ret, index=self.datelist, columns=[f'SHORT_{prefix}_{self.name}'])
        vw_short_df = pd.DataFrame(vw_low_ret, index=self.datelist, columns=[f'SHORT_{vw_prefix}_{self.name}'])
