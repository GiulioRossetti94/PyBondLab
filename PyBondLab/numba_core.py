...
@njit(cache=True, parallel=True)
def compute_within_firm_assignments_all_dates(
    date_idx: np.ndarray,       # Date index for each observation
    rating_terc: np.ndarray,    # Rating tercile (1, 2, 3) for each observation
    firm_idx: np.ndarray,       # Firm index for each observation
    signal: np.ndarray,         # Signal value for each observation
    vw: np.ndarray,             # Value weight for each observation
    group_starts: np.ndarray,   # Start index for each group
    group_ends: np.ndarray,     # End index for each group
    min_bonds_per_firm: int = 2
) -> np.ndarray:
    """
    Compute within-firm portfolio assignments for all dates in parallel.

    This is the Phase 16d ultra-fast kernel that processes all groups at once
    using numba prange for parallel execution.

    Each group is (date, rating_terc, firm) and bonds are assigned to:
    - LOW (1): signal < 33.33rd percentile of unique values
    - HIGH (2): signal > 66.67th percentile of unique values
    - Unassigned (0): middle tercile

    Parameters
    ----------
    date_idx : np.ndarray
        Date index for each observation (sorted by group)
    rating_terc : np.ndarray
        Rating tercile for each observation (sorted by group)
    firm_idx : np.ndarray
        Firm index for each observation (sorted by group)
    signal : np.ndarray
        Signal values for sorting (sorted by group)
    vw : np.ndarray
        Value weights (sorted by group)
    group_starts : np.ndarray
        Start indices for each group
    group_ends : np.ndarray
        End indices for each group
    min_bonds_per_firm : int
        Minimum bonds required per firm-date-rating group

    Returns
    -------
    portfolio_ranks : np.ndarray
    ...
    # Output array
    portfolio_ranks = np.zeros(n_obs, dtype=np.int32)
    ...
    for g in prange(n_groups):
        ...
        if n < min_bonds_per_firm:
            continue

        # Get signal and weight values for this group
        sig_slice = signal[start:end]
        w_slice = vw[start:end]

        # Check for finite values
        valid_count = 0
        for i in range(n):
            if np.isfinite(sig_slice[i]) and np.isfinite(w_slice[i]) and w_slice[i] > 0:
                valid_count += 1

        if valid_count < min_bonds_per_firm:
            continue

        # Collect valid signal values for percentile computation
        valid_signals = np.empty(valid_count, dtype=np.float64)
        j = 0
        for i in range(n):
            if np.isfinite(sig_slice[i]) and np.isfinite(w_slice[i]) and w_slice[i] > 0:
                valid_signals[j] = sig_slice[i]
                j += 1

        # Get unique values
        sorted_signals = np.sort(valid_signals)
        unique_signals = np.empty(valid_count, dtype=np.float64)
        n_unique = 0
        for i in range(valid_count):
            if n_unique == 0 or sorted_signals[i] != unique_signals[n_unique - 1]:
                unique_signals[n_unique] = sorted_signals[i]
                n_unique += 1

        if n_unique < 2:
            continue

        # Compute percentile thresholds (33.33% and 66.67%)
        # Use linear interpolation like np.percentile
        high_pct = 66.66666666666667
        low_pct = 33.33333333333333

        # Index for percentile (0-indexed)
        high_idx = (n_unique - 1) * high_pct / 100.0
        low_idx = (n_unique - 1) * low_pct / 100.0

        # Linear interpolation
        high_lower = int(high_idx)
        high_frac = high_idx - high_lower
        if high_lower >= n_unique - 1:
            high_bp = unique_signals[n_unique - 1]
        else:
            high_bp = unique_signals[high_lower] * (1 - high_frac) + unique_signals[high_lower + 1] * high_frac

        low_lower = int(low_idx)
        low_frac = low_idx - low_lower
        if low_lower >= n_unique - 1:
            low_bp = unique_signals[n_unique - 1]
        else:
            low_bp = unique_signals[low_lower] * (1 - low_frac) + unique_signals[low_lower + 1] * low_frac
        ...
        for i in range(n):
            ...
            if not (np.isfinite(s) and np.isfinite(w) and w > 0):
                continue

            if s > high_bp:
                portfolio_ranks[idx] = 2  # HIGH
                has_high = True
            elif s < low_bp:
                portfolio_ranks[idx] = 1  # LOW
                has_low = True

        # If either HIGH or LOW is empty, clear all assignments for this group
        if not (has_high and has_low):
            for i in range(n):
                portfolio_ranks[start + i] = 0
