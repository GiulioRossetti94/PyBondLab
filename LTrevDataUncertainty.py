...
for i, params in enumerate(params_all):
    ...
    ltrev_res = pbl.StrategyFormation(tbl1_loop, **params).fit()
    print(i+1,'/',len(params_all),"\t",ltrev_res.name)

    # get the long short portfolios ex ante and ex post
    ew_ea, vw_ea = ltrev_res.get_long_short() 
    ew_ep, vw_ep = ltrev_res.get_long_short_ex_post()
...
# Equal-Weight
pd.concat(l_ew_ea, axis = 1).to_csv(r'exante_LTrev_uncertainty_ew_' + RAT+'.csv')
pd.concat(l_ew_ep, axis = 1).to_csv(r'expost_LTrev_uncertainty_ew_' + RAT+'.csv')

# Value-Weight
pd.concat(l_vw_ea, axis = 1).to_csv(r'exante_LTrev_uncertainty_vw_' + RAT+'.csv')
pd.concat(l_vw_ep, axis = 1).to_csv(r'expost_LTrev_uncertainty_vw_' + RAT+'.csv')
# =============================================================================